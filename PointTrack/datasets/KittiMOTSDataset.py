...
class KITTI_raw_data_InsSeg_test(Dataset):
    """Get the test set of the KITTI raw data dataset for Instance Segmentation.
    """
    ...
    def __init__(self, root_dir='./', type="train", class_id=26, size=None, transform=None, batch=False, batch_num=8):
        ...
        type = 'training' if type in 'training' else 'testing' # 如果初始化参数type=='train'或type=='training'，则self.type='training'，对应训练集，否则self.type='testing'，对应验证集/测试集
        self.type = type
        ...
        self.mots_image_root = os.path.join(kittiRoot, 'raw_data')
        self.timestamps = self.TIMESTEPS_PER_SEQ_TEST

        self.mots_car_pairs = [] # 用于存储KITTI raw data的文件路径，元素格式：‘[年_月_日]/[年_月_日_drive_视频索引_sync]/image_02/data/[图像索引].png’
        for subdir in self.timestamps.keys():
            image_list = sorted(make_dataset(os.path.join(self.mots_image_root, "_".join(subdir.split("_")[:3]), subdir, 'image_02/data'), suffix='.png'))
            image_list = ['/'.join(el.split('/')[-5:]) for el in image_list]
            self.mots_car_pairs += image_list

        self.mots_num = len(self.mots_car_pairs) # KITTI raw data的图像个数
...
class KITTI_raw_data_Tracking_test(Dataset):
    """Get the test set of the KITTI raw data dataset for Tracking.
    """
    TIMESTEPS_PER_SEQ_TEST = {'2011_09_26_drive_0001_sync': 108, '2011_09_26_drive_0002_sync': 77, '2011_09_26_drive_0005_sync': 154,
                              '2011_09_26_drive_0009_sync': 447, '2011_09_26_drive_0011_sync': 233, '2011_09_26_drive_0013_sync': 144,
                              '2011_09_26_drive_0014_sync': 314, '2011_09_26_drive_0015_sync': 297, '2011_09_26_drive_0017_sync': 114,
    ...
    def __init__(self, root_dir='./', type="train", num_points=250, transform=None, random_select=False, az=False,
        ...
        self.timestamps = self.TIMESTEPS_PER_SEQ_TEST
        self.image_root = os.path.join(kittiRoot, 'raw_data')
        self.mots_root = os.path.join(rootDir, 'car_KITTI_raw_data_SpatialEmbedding_test_prediction')

        print('use ', self.mots_root)
        self.batch_num = 2

        self.mots_car_sequence = [] # 用于存储KITTI raw data的SpatialEmbeddings预测文件路径，元素格式：‘self.mots_root/[年_月_日_drive_视频索引_sync]_图像索引.pkl’
        for subdir in self.timestamps.keys():
            nums = self.timestamps[subdir]
            for i in range(nums):
                pklPath = os.path.join(self.mots_root, subdir + '_' + str(i) + '.pkl')
                if os.path.isfile(pklPath):
                    self.mots_car_sequence.append(pklPath)

        self.real_size = len(self.mots_car_sequence) # KITTI raw data的图像个数
        self.mots_num = len(self.mots_car_sequence) # KITTI raw data的图像个数
        self.mots_class_id = 1 # Car的类别标签
        self.expand_ratio = ex
        self.vMax, self.uMax = 375.0, 1242.0
        self.num_points = num_points # crop内前景、背景点云总点数
        self.env_points = 200 # 环境点云最大点数
    ...
    def get_data_from_mots(self, index):
        ...
        subdir, frameCount = "_".join(os.path.basename(path)[:-4].split('_')[:-1]), os.path.basename(path)[:-4].split('_')[-1]
        imgPath = os.path.join(self.image_root, "_".join(subdir.split("_")[:3]), subdir, 'image_02/data', '%010d.png' % int(float(frameCount)))
        ...
        for inst_id in inds:
            ...
            mask, img_, maskX, sp = self.get_crop_from_mask(mask, img, label.copy()) # crop内的mask、img、label、crop左上角坐标
            # fg/bg ratio
            ratio = 2.0
            # ratio = max(mask.sum() / (~mask).sum(), 2.0)
            bg_num = int(self.num_points / (ratio + 1)) # 背景点云的点数
            fg_num = self.num_points - bg_num # 前景点云的点数

            vs_, us_ = np.nonzero(mask)
            vc, uc = vs_.mean(), us_.mean() # 实例中心

            vs = (vs_ - vc) / self.offsetMax # v坐标归一化偏移量
            us = (us_ - uc) / self.offsetMax # u坐标归一化偏移量
            rgbs = img_[mask] / 255.0 # 归一化RGB颜色
            ...
            if (~mask).sum() == 0:
                ...
                vs, us = np.nonzero(~mask)
                vs = (vs - vc) / self.offsetMax # v坐标归一化偏移量
                us = (us - uc) / self.offsetMax # u坐标归一化偏移量
                rgbs = img_[~mask] / 255.0 # 归一化RGB颜色
                cats = maskX[~mask] # 类别one-hot编码
        ...
        if len(sample['points']) > 0:
            sample['points'] = np.concatenate(sample['points'], axis=0) # shape: (num_instances, num_points, num_features)
            sample['masks'] = np.concatenate(sample['masks'], axis=0) # shape: (num_instances, img.shape[0], img.shape[1])
            sample['envs'] = np.array(sample["envs"], dtype=np.int32) # shape: (num_instances, )
            sample['xyxys'] = np.array(sample["xyxys"], dtype=np.float32) # shape: (num_instances, 4)
        return sample
...
class MOTSTest(Dataset):
    """Get the test set of the KITTI tracking dataset.
    """
    SEQ_IDS_TEST = ["%04d" % idx for idx in range(29)] # KITTI tracking测试集视频序列文件夹名
    TIMESTEPS_PER_SEQ_TEST = {'0000': 465, '0015': 701, '0017': 305, '0003': 257, '0001': 147, '0018': 180, '0005': 809,
                              ...
                              '0002': 243} # KITTI tracking测试集每个视频序列包含的帧数（图像文件个数）
    ...
    def __init__(self, root_dir='./', type="train", class_id=26, size=None, transform=None, batch=False, batch_num=8):
        ...
        self.mots_car_pairs = [] # 用于存储KITTI tracking测试集的文件路径，元素格式：‘/视频索引/图像索引.png’
        ...
        self.mots_num = len(self.mots_car_pairs) # KITTI tracking测试集的图像个数
...
class MOTSCarsVal(Dataset):
    """Obtain the training set/validation set of the KITTI tracking dataset,
    and only contain the Car category.
    """
    SEQ_IDS_TRAIN = ["%04d" % idx for idx in [0, 1, 3, 4, 5, 9, 11, 12, 15, 17, 19, 20]] # KITTI tracking训练集视频序列文件夹名
    SEQ_IDS_VAL = ["%04d" % idx for idx in [2, 6, 7, 8, 10, 13, 14, 16, 18]] # KITTI tracking验证集视频序列文件夹名

    def __init__(self, root_dir='./', type="train", class_id=26, size=None, transform=None, batch=False, batch_num=8):

        print('Kitti Dataset created')
        type = 'training' if type in 'training' else 'testing' # 如果初始化参数type=='train'或type=='training'，则self.type='training'，对应训练集，否则self.type='testing'，对应验证集
        self.type = type
        self.sequence = self.SEQ_IDS_TRAIN if type == 'training' else self.SEQ_IDS_VAL # 如果self.type=='training'，对应KITTI tracking训练集视频序列文件夹名，否则self.type='testing'，对应KITTI tracking验证集视频序列文件夹名
        self.batch = batch
        self.batch_num = batch_num
        ...
        self.mots_instance_root = os.path.join(kittiRoot, 'instances') # KITTI tracking training的目标可见部分实例分割mask标签
        self.mots_image_root = os.path.join(kittiRoot, 'images')

        self.mots_car_pairs = [] # 用于存储KITTI tracking训练集/验证集的文件路径，同样对应实例分割mask标签的文件路径，元素格式：‘/视频索引/图像索引.png’
        ...
        self.mots_num = len(self.mots_car_pairs) # KITTI tracking训练集/验证集的图像个数
        self.mots_class_id = 1 # Car的类别标签
    ...
    def get_data_from_mots(self, index):
        ...
        sample['mot_image'] = image
        sample['mot_instance'] = instance # 实例分割mask标签
        sample['mot_label'] = label # 语义分割mask标签，Car: 1，Dontcare: -2，else: 0
        sample['mot_im_name'] = path
        sample['im_shape'] = image.size
...
class MOTSTrackCarsValOffset(Dataset):
    ...
    TIMESTEPS_PER_SEQ = {"0000": 154, "0001": 447, "0002": 233, "0003": 144, "0004": 314, "0005": 297, "0006": 270,
                         ...
                         "0014": 106, "0015": 376, "0016": 209, "0017": 145, "0018": 339, "0019": 1059, "0020": 837} # 共8008个图像
    SEQ_IDS_TEST = ["%04d" % idx for idx in range(29)] # KITTI tracking测试集视频序列文件夹名
    TIMESTEPS_PER_SEQ_TEST = {'0000': 465, '0015': 701, '0017': 305, '0003': 257, '0001': 147, '0018': 180, '0005': 809,
                              '0022': 436, '0021': 203, '0023': 430, '0012': 694, '0008': 165, '0009': 349, '0020': 173,
                              '0016': 510, '0013': 152, '0004': 421, '0028': 175, '0024': 316, '0019': 404, '0026': 170,
    ...
    def __init__(self, root_dir='./', type="train", num_points=250, transform=None, random_select=False, az=False,
        ...
        self.mots_car_sequence = [] # 用于存储KITTI tracking验证集/测试集的SpatialEmbeddings预测文件路径，元素格式：‘self.mots_root/视频索引_图像索引.pkl’
        for valF in ids:
        ...
        self.real_size = len(self.mots_car_sequence) # KITTI tracking验证集/测试集的图像个数
        self.mots_num = len(self.mots_car_sequence) # KITTI tracking验证集/测试集的图像个数
        self.mots_class_id = 1 # Car的类别标签
        self.expand_ratio = ex
        self.vMax, self.uMax = 375.0, 1242.0
        self.num_points = num_points # crop内前景、背景点云总点数
...
class MOTSTrackCarsTrain(Dataset):
    ...
    def __init__(self, root_dir='./', type="train", num_points=250, transform=None, random_select=False, batch_num=8,
        ...
        for id in self.squence:
            ...
            image_list.sort()
            infos = {} # 用于存储KITTI tracking训练集/验证集/训练验证集当前视频序列的ImgTrackEnvDB文件路径，元素格式：‘db_dir/视频索引_图像索引.pkl’
