...
ym = torch.linspace(0, 1, 1024).view(
    1, -1, 1).expand(1, 1024, 2048)
xym = torch.cat((xm, ym), 0).cuda() # shape: (2, 1024, 2048)
...
with torch.no_grad():
    ...
    for i, sample in enumerate(tqdm(dataset_it)):
        base = "_".join([sample['mot_im_name'][0].split('/')[1], sample['mot_im_name'][0].split('/')[-1]]).replace('.png', '.pkl') # 元素格式：‘[年_月_日]/[年_月_日_drive_视频索引_sync]/image_02/data/[图像索引].png’
        middle = base[:-4].split('_')[-1]
        base = base.replace(middle, str(int(float(middle)))) # 元素格式：‘[年_月_日_drive_视频索引_sync]_图像索引.pkl’
