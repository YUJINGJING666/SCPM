...
def obtain_cluster_box(l_3d, l_2d, seg_bbox_path, seg_mask_path, P2, wh_range, mask_conf, invalid_counts):
    ...
    for index, b in enumerate(bbox2d):
        ...
        if len(cam_points) < 10: # 目标因ROI点云数量过少而被过滤
            invalid_counts[2] += 1
            continue

        cluster_index = DBSCAN(eps=0.8, min_samples=10, n_jobs=-1).fit_predict(cam_points)

        cam_points = cam_points[cluster_index > -1] # 成功聚类点云
        cluster_index = cluster_index[cluster_index > -1]

        if len(cam_points) < 10: # 成功聚类点云数量过少
            invalid_counts[3] += 1
            continue

        cluster_set = set(cluster_index[cluster_index > -1])
        cluster_sum = np.array([len(cam_points[cluster_index == i]) for i in cluster_set])
        cam_points = cam_points[cluster_index == np.argmax(cluster_sum)] # 目标点云

        rect = cv.minAreaRect(np.array([(cam_points[:, [0, 2]]).astype(np.float32)])) # 计算BEV最小外接矩形
