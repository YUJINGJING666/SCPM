...
dst_high_acc_label_dir = os.path.join(kitti_cfg['root_dir'], 'high_acc/raw_label_2') # raw_data伪标签存储文件夹的目标路径
...
def main():
    ...
    with torch.no_grad():
        for idx, data_dict in enumerate(tqdm(demo_dataset)): # 遍历raw_data点云数据，预测生成伪标签
            ...
            valid_ind = np.array([i in cls_ind.keys() for i in pred_labels]) # 前景的索引
            ...
            loc, hwl, ry, cls, scr = loc[valid_ind], hwl[valid_ind], ry[valid_ind], pred_labels[valid_ind], pred_scores[valid_ind]
            ...
            if np.sum(bbox_ind) > 0:
                ...
                pred = np.concatenate([np.zeros((bbox.shape[0], 3), dtype=np.float32), bbox, hwl, loc, ry, scr[:, np.newaxis]], axis=1)
                res = ["{} {} {} {} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f} {:.2f}".format(i, *j)
