...
predres_path = "/mnt/disk1/datasets/KITTI/dataset_for_SaVos/results/Kitti_car_track_with_track_split_200/log_pred_res" # 没有轨迹划分："dataset/data/Kitti/Kitti_car_track"，轨迹划分："dataset/data/Kitti/Kitti_car_track_with_track_split"
...
for idx, img_path in enumerate(img_list):
    ...
    if len(bbox) > 0:
        ...
        np.savez(seg_mask_path, masks=masks)
        valid_count += 1
    else:
        empty_count += 1

invalid_count = nonexist_count + empty_count

print("There are %d valid amodal masks, %d nonexistent amodal masks, %d empty amodal masks, %d invalid amodal masks!" % (valid_count, nonexist_count, empty_count, invalid_count))
print("There are %d valid objects, %d invalid objects!" % (obj_count, invalid_obj_count))