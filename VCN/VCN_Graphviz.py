...
from utils.util_flow import write_flow, save_pfm, ConvertKittiPngToMiddleburyFlo
...
parser.add_argument('--reverse', action="store_true",
                    help='time order of optical flow')
...
elif args.dataset == 'kittirawdata':
    from dataloader import KITTI_raw_data as DA
    maxw,maxh = [int(args.testres*1280), int(args.testres*384)]
    test_left_img, test_right_img, _= DA.dataloader(args.datapath, args.reverse)
...
def main():
    ...
    for inx in tqdm(range(len(test_left_img))):
        ...
        imgL_o = skimage.io.imread(test_left_img[inx]) # np.adarray(), size: (maxh, maxw, 3), RGB
        imgR_o = skimage.io.imread(test_right_img[inx]) # np.adarray(), size: (maxh, maxw, 3), RGB
        ...
        imgL = cv2.resize(imgL_o,(max_w, max_h)) # np.adarray(), size: (max_h, max_w, 3), RGB
        imgR = cv2.resize(imgR_o,(max_w, max_h)) # np.adarray(), size: (max_h, max_w, 3), RGB
        ...
        imgL = np.transpose(imgL, [2,0,1])[np.newaxis] # np.adarray(), size: (1, 3, max_h, max_w), RGB
        imgR = np.transpose(imgR, [2,0,1])[np.newaxis] # np.adarray(), size: (1, 3, max_h, max_w), RGB
        ...
        # forward
        imgL = Variable(torch.FloatTensor(imgL).cuda()) # torch.Tensor(), size: (1, 3, max_h, max_w), RGB
        imgR = Variable(torch.FloatTensor(imgR).cuda()) # torch.Tensor(), size: (1, 3, max_h, max_w), RGB
        with torch.no_grad():
            imgLR = torch.cat([imgL,imgR],0) # torch.Tensor(), size: (2, 3, max_h, max_w), RGB
            model.eval()
            ...
            pred_disp, entropy = rts # torch.Tensor(), size: (1, 2, max_h, max_w), (x, y), torch.Tensor(), size: (1, max_h, max_w)

            # VCNVis = make_dot(rts, params=dict(list(model.named_parameters()) + [('imgLR', imgLR)]))
            # VCNVis.format = "png"
            # # 指定文件生成的文件夹
            # VCNVis.directory = "graphviz"
            # # 生成文件
            # VCNVis.render("VCN", view=False)
    ...
    #     # upsampling
    #     pred_disp = torch.squeeze(pred_disp).data.cpu().numpy() # np.adarray(), size: (2, max_h, max_w), (x, y)
    #     pred_disp = cv2.resize(np.transpose(pred_disp,(1,2,0)), (input_size[1], input_size[0])) # np.adarray(), size: (maxh, maxw, 2), (x, y)
    #     pred_disp[:,:,0] *= input_size[1] / max_w
    #     pred_disp[:,:,1] *= input_size[0] / max_h
    #     flow = np.ones([pred_disp.shape[0],pred_disp.shape[1],3]) # np.adarray(), size: (maxh, maxw, 3), (1, 1, 1)
    #     flow[:,:,:2] = pred_disp # np.adarray(), size: (maxh, maxw, 3), (x, y, 1)
    #     entropy = torch.squeeze(entropy).data.cpu().numpy() # np.adarray(), size: (max_h, max_w)
    #     entropy = cv2.resize(entropy, (input_size[1], input_size[0])) # np.adarray(), size: (maxh, maxw)
    ...
    #     elif args.dataset == 'kittirawdata':
    #         if args.reverse:
    #             png_filepath = os.path.join(args.outdir, args.dataset, '/'.join(test_right_img[inx].split('/')[-5:])).replace('image_02/data', 'reverse_flow/png_file')
    #         else:
    #             png_filepath = os.path.join(args.outdir, args.dataset, '/'.join(test_left_img[inx].split('/')[-5:])).replace('image_02/data', 'flow/png_file')
    #         # flo_filepath = png_filepath.replace('png_file', 'flo_file').replace('png', 'flo')
    #         if not os.path.exists(png_filepath[:-15]):
    #             os.makedirs(png_filepath[:-15])
    #         # if not os.path.exists(flo_filepath[:-15]):
    #         #     os.makedirs(flo_filepath[:-15])
    #         write_flow(png_filepath, flow.copy())
    #         # ConvertKittiPngToMiddleburyFlo(png_filepath, flo_filepath)
    #         # os.remove(png_filepath)
    #     else:
    #         write_flow('%s/%s/%s.png'% (args.outdir, args.dataset, idxname.rsplit('.',1)[0]), flow.copy())
    #     if args.dataset == 'kittirawdata':
    #         ent_png_filepath = png_filepath.replace('png_file', 'ent_png_file')
    #         if not os.path.exists(ent_png_filepath[:-15]):
    #             os.makedirs(ent_png_filepath[:-15])
    #         cv2.imwrite(ent_png_filepath, entropy*200)
    #     else:
