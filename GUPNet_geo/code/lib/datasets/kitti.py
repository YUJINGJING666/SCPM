    ...
    def __getitem__(self, item):
        ...
        if self.data_augmentation:
            ...
            if np.random.random() < self.random_crop:
                ...
                scalar = self.resolution[0] / crop_size[0] # 新加入的scalar因子
        ...
        if self.split!='test':
            ...
            lidar_depth = np.zeros((self.max_objs, 1), dtype=np.float32)
            att_depth = np.zeros((self.max_objs, 1), dtype=np.float32)
            depth = np.zeros((self.max_objs, 1), dtype=np.float32)
            rotation = np.zeros((self.max_objs, 1), dtype=np.float32)
            ...
            for i in range(object_num):
                ...
                rotation[i] = objects[i].ry

                # encoding depth
                depth[i] = objects[i].pos[-1]
                flag = (np.abs(np.tan(rotation[i]))<(objects[i].l/objects[i].w))
                if flag:
                    att_depth[i] = objects[i].w/(2*np.abs(np.cos(rotation[i])))
                else:
                    att_depth[i] = objects[i].l/(2*np.abs(np.sin(rotation[i])))
                #att_depth[i] = objects[i].w/(2*np.abs(np.cos(rotation[i])))*flag + objects[i].l/(2*np.abs(np.sin(rotation[i])))*(1-flag)
                lidar_depth[i] = np.clip(depth[i]-att_depth[i], 1, 1000)
                if random_crop_flag:
                    lidar_depth[i] = lidar_depth[i] / scalar # 若scalar>1 说明裁剪后放大 即深度拉进
                depth[i] = lidar_depth[i] + att_depth[i]
                ...
                if objects[i].trucation <=0.5 and objects[i].occlusion<=2:    
                    mask_2d[i] = 1           
            targets = {'lidar_depth': lidar_depth,
                    'att_depth': att_depth,
                    'depth': depth,
                   'rotation': rotation,
                   'size_2d': size_2d,
