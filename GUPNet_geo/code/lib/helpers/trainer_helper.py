...
class Trainer(object):
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.cfg_train['max_epoch']):
            ...
            if (self.epoch % self.cfg_train['save_frequency']) == 0:
                os.makedirs(self.cfg_train['log_dir']+'/checkpoints_trainval', exist_ok=True) # 验证集上评估：'/checkpoints', 测试集上评估：'/checkpoints_trainval'
                ckpt_name = os.path.join(self.cfg_train['log_dir']+'/checkpoints_trainval', 'checkpoint_epoch_%d' % self.epoch) # 验证集上评估：'/checkpoints', 测试集上评估：'/checkpoints_trainval'
    ...
    def save_results(self, results, output_dir='./outputs_test'): # 验证集上评估：'./outputs', 测试集上评估：'./outputs_test'
