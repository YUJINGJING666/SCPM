...
class GupnetLoss(nn.Module):
    ...
    def compute_bbox3d_loss(self, input, target, mask_type = 'mask_2d'):
        ...
        # compute depth loss        
        lidar_depth_input = input['lidar_depth'][input['train_tag']] 
        lidar_depth_input, lidar_depth_log_variance = lidar_depth_input[:, 0:1], lidar_depth_input[:, 1:2]
        lidar_depth_target = extract_target_from_tensor(target['lidar_depth'], target[mask_type])
        lidar_depth_loss = laplacian_aleatoric_uncertainty_loss(lidar_depth_input, lidar_depth_target, lidar_depth_log_variance)
        ...
        rotation_input = input['rotation'][input['train_tag']] 
        rotation_target = extract_target_from_tensor(target['rotation'], target[mask_type])
        rotation_loss = F.l1_loss(rotation_input, rotation_target, reduction='mean')
        ...
        #size3d_loss = laplacian_aleatoric_uncertainty_loss(size3d_input[:,1:2], size3d_target[:,1:2], input['w3d_log_variance'][input['train_tag']])/3+\
        #       laplacian_aleatoric_uncertainty_loss(size3d_input[:,0:1], size3d_target[:,0:1], input['h3d_log_variance'][input['train_tag']])/3+\
        #       laplacian_aleatoric_uncertainty_loss(size3d_input[:,2:3], size3d_target[:,2:3], input['l3d_log_variance'][input['train_tag']])/3
        ...
        heading_loss = compute_heading_loss(input['heading'][input['train_tag']] ,
                                            ...
                                            target['heading_bin'],
                                            target['heading_res'])
        loss = depth_loss + lidar_depth_loss + offset3d_loss + size3d_loss + heading_loss + rotation_loss
        
        self.stat['rotation_loss'] = rotation_loss
        self.stat['lidar_depth_loss'] = lidar_depth_loss
