...
class GUPNet(nn.Module):
    def __init__(self, backbone='dla34', neck='DLAUp', downsample=4, mean_size=None):
        ...
        self.rotation = nn.Sequential(nn.Conv2d(channels[self.first_level]+2+self.cls_num, self.head_conv, kernel_size=3, padding=1, bias=True),
                                     nn.BatchNorm2d(self.head_conv),
                                     nn.ReLU(inplace=True),nn.AdaptiveAvgPool2d(1),
                                     nn.Conv2d(self.head_conv, 1, kernel_size=1, stride=1, padding=0, bias=True))
        self.lidar_depth = nn.Sequential(nn.Conv2d(channels[self.first_level]+2+self.cls_num, self.head_conv, kernel_size=3, padding=1, bias=True),
                                     nn.BatchNorm2d(self.head_conv),
                                     nn.ReLU(inplace=True),nn.AdaptiveAvgPool2d(1),
                                     nn.Conv2d(self.head_conv, 1, kernel_size=1, stride=1, padding=0, bias=True))
        ...
        #                              nn.Conv2d(self.head_conv, 1, kernel_size=1, stride=1, padding=0, bias=True))
        self.lidar_depth_uncer = nn.Sequential(nn.Conv2d(channels[self.first_level]+2+self.cls_num, self.head_conv, kernel_size=3, padding=1, bias=True),
                                     nn.BatchNorm2d(self.head_conv),
                                     nn.ReLU(inplace=True),nn.AdaptiveAvgPool2d(1),
                                     nn.Conv2d(self.head_conv, 1, kernel_size=1, stride=1, padding=0, bias=True))
        ...
        self.rotation.apply(weights_init_xavier)

        self.lidar_depth.apply(weights_init_xavier)
        #self.att_depth.apply(weights_init_xavier)
        self.lidar_depth_uncer.apply(weights_init_xavier)
    ...
    def forward(self, input, coord_ranges,calibs, targets=None, K=50, mode='train'):
        ...
        ret.update(self.get_roi_feat(feat,inds,masks,ret,calibs,coord_ranges,cls_ids,mode))
        return ret

    def get_roi_feat_by_mask(self,feat,box2d_maps,inds,mask,calibs,coord_ranges,cls_ids,mode='train'):
        ...
        if num_masked_bin!=0:
            ...
            scale_box2d_masked = extract_input_from_tensor(box2d_maps,inds,mask) # 原始DID
            #box2d_masked = extract_input_from_tensor(box2d_maps,inds,mask) # 原始GUPNet
            #get roi feature
            ...
            #w3d_std = size3d_offset[:,4:5].exp()
            #l3d_std = size3d_offset[:,5:6].exp()
            size3d_offset = size3d_offset[:,:3]  # h,w,l

            size_3d = (self.mean_size[cls_ids[mask].long()]+size3d_offset)# h,w,l
            w = size_3d[:,1].view(-1, 1)
            l = size_3d[:,2].view(-1, 1)
            #print('l:', l.shape) # [n]

            rotation = self.rotation(roi_feature_masked)[:,:,0,0][:,0:1]
            r = rotation # 本版本改动去掉了l,w,r中的detach
            #print('rotation:', rotation.shape) # [n, 1]
            ...
            scale_depth = scale_depth.view(-1, 1)
            #print('scale_depth:', scale_depth.shape)

            lidar_depth = self.lidar_depth(roi_feature_masked)[:,:,0,0]

            att_flag =  (torch.abs(torch.tan(r)) < l/w).type(torch.float32)
            #print('att_flag:', att_flag.shape) # [n, n]
            att_depth = w/(2*torch.abs(torch.cos(rotation))+1e-5)*att_flag + l/(2*torch.abs(torch.sin(rotation))+1e-5)*(1-att_flag)
            #att_depth = w/(2*torch.abs(torch.cos(r)))*att_flag + l/(2*torch.abs(torch.sin(r)))*(1-att_flag)
            #print('att_depth:',att_depth.shape) # [n, n]
            #att_depth_uncer = (w3d_std/(2*torch.abs(torch.cos(rotation))+1e-5)*att_flag + l3d_std/(2*torch.abs(torch.sin(rotation))+1e-5)*(1-att_flag)+1e-4).log()
            
            #print('att_uncer:',att_depth_uncer.shape) # [n, n]

            lidar_depth = (-lidar_depth).exp()
            #print('lidar_depth_0:', lidar_depth.shape)
            if mode == 'train':
                lidar_depth = lidar_depth * scale_depth
            else:
                lidar_depth = lidar_depth
            #print(mode)
            #print('lidar_depth:',lidar_depth.shape)  # [n, n, 1]
            lidar_depth_uncer = self.lidar_depth_uncer(roi_feature_masked)[:, 0, :, :].squeeze(-1)
            #print('lidar_uncer:',lidar_depth_uncer.shape)  # [n, 1, 1]
            
            ins_depth = lidar_depth + att_depth
            #depth_uncer = torch.logsumexp(torch.stack([lidar_depth_uncer, att_depth_uncer], -1), -1)
            depth_uncer = lidar_depth_uncer
            depth = torch.cat([ins_depth, depth_uncer], -1)
            lidar_depth_out = torch.cat([lidar_depth, lidar_depth_uncer], -1)
            '''
           ...
           # res['l3d_log_variance'] = l3d_std.log()
            ...
            #res['w3d_log_variance'] = torch.zeros([1,1]).to(device_id)
            #res['l3d_log_variance'] = torch.zeros([1,1]).to(device_id)
            res['rotation'] = torch.zeros([1,1]).to(device_id)
        return res

    def get_roi_feat(self,feat,inds,mask,ret,calibs,coord_ranges,cls_ids,mode='train'):
        BATCH_SIZE,_,HEIGHT,WIDE = feat.size()
        device_id = feat.device
        ...
        res = self.get_roi_feat_by_mask(feat,box2d_maps,inds,mask,calibs,coord_ranges,cls_ids,mode)
        return res
